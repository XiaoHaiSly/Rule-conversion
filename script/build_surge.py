...
# 输出到 rules/surge，与 rules/mihomo、rules/singbox 平级
RULES_LINKS_FILES = [
    "../links/links-domain.txt",
    "../links/links-ipcidr.txt",
    "../links/links-mixed.txt",
]
RULES_OUTPUT_ROOT = "../rules/surge"

# 自动从 MetaCubeX/meta-rules-dat 拉取的全量 geosite+geoip（links-meta.txt）
# 输出到 geo-data/surge，与 geo-data/mihomo、geo-data/singbox 平级
GEO_LINKS_FILES = [
    "../links/links-meta.txt",
]
GEO_OUTPUT_ROOT = "../geo-data/surge"
...
def build_group(name, links, work_dir, output_root):
    ...
    for link in links:
        ...
        if unified == 'UNSUPPORTED':
            print(f"[跳过] {link}：无法从 mrs 反解，Surge 规则无法导入")
            continue
        if not unified:
    ...
    list_path = os.path.join(out_dir, f"{name}.list")
    if common.write_surge_list(domain_part, ipcidr_part, list_path):
        print(f"[完成] {name} -> {output_root}/{name}/{name}.list，共 {len(links)} 个源")
    else:
